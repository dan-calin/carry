...
const MAX_SESSION_MEMORY_RECORDS = 1000;
...
function stateFile(root, peerId) {
  return privateState.projectFile(root, 'state', safePeerId(peerId) + '.json');
}

function resolutionFile(root, peerId) {
  return privateState.projectFile(root, 'resolutions', safePeerId(peerId) + '.json');
}
...
function sessionFile(root, sessionId) {
  ...
  return privateState.projectFile(root, 'sessions', safe + '.json');
}
...
function sessionMemoryChanges(result, status) {
  if (!result) return null;
  const entities = (result.addedEntityRecords || [])
    .slice(0, MAX_SESSION_MEMORY_RECORDS)
    .map((item) => ({ name: item.name, entityType: item.entityType || 'entity' }));
  const observations = (result.addedObservationRecords || [])
    .slice(0, MAX_SESSION_MEMORY_RECORDS)
    .map((item) => ({ name: item.name, text: item.observation }));
  const relations = (result.addedRelationRecords || [])
    .slice(0, MAX_SESSION_MEMORY_RECORDS)
    .map((item) => ({ from: item.from, to: item.to, relationType: item.relationType }));
  return {
    status,
    entityNames: result.changedEntityNames.slice(0, MAX_SESSION_MEMORY_RECORDS),
    totalEntities: result.changedEntityNames.length,
    addedEntities: result.addedEntities,
    addedObservations: result.addedObs,
    addedRelations: result.addedRels,
    entities,
    observations,
    relations,
    recordsTruncated:
      (result.addedEntityRecords || []).length > entities.length ||
      (result.addedObservationRecords || []).length > observations.length ||
      (result.addedRelationRecords || []).length > relations.length,
  };
}
...
function backupFile(root, sessionId, rel, source, checkpointHash) {
  ...
  const target = privateState.projectFile(root, 'backups', sessionId, ...rel.split('/'));
  ...
  return path.posix.join('.carry', 'backups', sessionId, rel);
}
...
function prepareIncoming(root, peerId, bundle, selfAgent, options) {
  ...
  const incomingRoot = path.resolve(privateState.projectFile(root, 'incoming'));
  ...
  const automaticCheckpoint = changesLocalProject || memoryWillChange
    ? checkpoints.create(root, `Before sync from ${String(peerId).slice(0, 8)}`, {
      kind: 'automatic',
      sourcePeerId: String(peerId),
      memoryEntityNames: memoryPreview ? memoryPreview.changedEntityNames : [],
    })
    : null;
  const backups = [];
  ...
  const originalManifest = activeDevice
    ? fs.readFileSync(manifest.manifestFile(root))
    : null;
  ...
  writeJsonAtomic(sessionPath, {
    ...
    actions,
    directoryActions,
    memory: sessionMemoryChanges(memoryPreview, 'pending'),
  });
  ...
  for (const item of actions) {
    ...
    const copyBase = privateState.projectFile(root, 'conflicts', sessionId, ...item.path.split('/'));
    if (item.local !== null) {
      ...
      copies.local = path.posix.join('.carry', 'conflicts', sessionId, item.path + '.local');
    }
    if (item.remote !== null) {
      ...
      copies.remote = path.posix.join('.carry', 'conflicts', sessionId, item.path + '.remote');
    }
  ...
  result.memory = sessionMemoryChanges(
    memoryResult,
    memoryResult ? (memoryResult.changed ? 'changed' : 'unchanged') : 'unchanged',
  );
  ...
  return result;
    ...
    try {
      ...
      if (activeDeviceMerged && originalManifest) {
        writeFileAtomic(manifest.manifestFile(root), originalManifest);
      }
    } catch (caught) {
...
function conflictSnapshotFile(root, relative) {
  ...
  const suffix = normalized.slice('.carry/conflicts/'.length);
  const prefix = path.resolve(privateState.projectFile(root, 'conflicts'));
...
function listSessions(root, limit) {
  const dir = privateState.projectFile(root, 'sessions');
