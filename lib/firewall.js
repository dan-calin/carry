...
function conciseFailure(result, elevatedStatus) {
  const raw = elevatedStatus && elevatedStatus.error ||
...
function installLanRules(opts) {
  ...
  const statusPath = opts.statusPath || path.join(os.tmpdir(),
    `carry-firewall-${process.pid}-${crypto.randomBytes(8).toString('hex')}.json`);
  const elevatedScript = [
    "$ErrorActionPreference = 'Stop'",
    `$statusPath = '${psQuote(statusPath)}'`,
    '$succeeded = $false',
    '$detail = $null',
    'try {',
    ...
    '}',
    'try {',
    '  $json = @{ ok = $succeeded; error = $detail } | ConvertTo-Json -Compress',
  ...
  let elevatedStatus = null;
  try {
    elevatedStatus = JSON.parse(fs.readFileSync(statusPath, 'utf8'));
  } catch { /* the elevated process may have been cancelled before writing */ }
  try { fs.unlinkSync(statusPath); } catch { /* best-effort private status cleanup */ }

  const commandSucceeded = !result.error && result.status === 0 && elevatedStatus?.ok !== false;
  ...
  return {
    ...
    message: ok ? null : conciseFailure(result, elevatedStatus),
  };
}
