...
// Carry project metadata is stored in the current user's local application-data
// directory, keyed by the canonical project path. It is never placed beside
// files that may be shared, backed up, or synchronized by another product.
function carryDir(root) {
  return privateState.projectDir(root);
}
...
function findCarryRoot(start) {
  ...
  for (;;) {
    const stateDir = privateState.existingProjectDir(dir);
    const candidate = stateDir && path.join(stateDir, MANIFEST);
    if (candidate && fs.existsSync(candidate) && fs.lstatSync(candidate).isFile() &&
        !fs.lstatSync(candidate).isSymbolicLink()) return dir;
...
function readManifest(root) {
  ...
  try {
    ...
    let needsCredentialUpgrade = false;
    for (const peer of Object.values(doc.peers || {})) {
      if (Object.prototype.hasOwnProperty.call(peer, 'pairCodeProtected')) {
        peer.pairCode = privateState.unprotectSecret(peer.pairCodeProtected);
        delete peer.pairCodeProtected;
      } else if (peer.pairCode !== null && peer.pairCode !== undefined) {
        needsCredentialUpgrade = true;
      }
      if (Object.prototype.hasOwnProperty.call(peer, 'teamCodeProtected')) {
        peer.teamCode = privateState.unprotectSecret(peer.teamCodeProtected);
        delete peer.teamCodeProtected;
      } else if (peer.teamCode !== null && peer.teamCode !== undefined) {
        needsCredentialUpgrade = true;
      }
    }
    const validated = validateManifest(doc);
    if (needsCredentialUpgrade) writeManifest(root, validated);
    return validated;
  } catch (error) {
...
function writeManifest(root, doc) {
  ...
  for (const peer of Object.values(stored.peers || {})) {
    peer.pairCodeProtected = privateState.protectSecret(peer.pairCode);
    peer.teamCodeProtected = privateState.protectSecret(peer.teamCode);
    delete peer.pairCode;
    delete peer.teamCode;
  }
  writeFileAtomic(manifestFile(root), JSON.stringify(stored, null, 2) + '\n');
}
