...
function decryptFrame(env, code) {
...
function encryptBinaryFrame(frame, payload, code, salt) {
  ...
  const key = sessionKey(code, salt, BINARY_INFO);
  const cipher = crypto.createCipheriv('aes-256-gcm', key, nonce);
...
function decryptBinaryFrame(envelope, code) {
